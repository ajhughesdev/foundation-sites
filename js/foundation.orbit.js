...
class Orbit extends Plugin {
  ...
  _setWrapperHeight(cb) {
    ...
    this.$slides.each(function () {
      ...
      if (
        !/mui/g.test($(this)[0].className) &&
        _this.$slides.filter('.is-active')[0] !== _this.$slides.eq(counter)[0]
      ) {
  ...
  _events() {
    ...
    if (this.$slides.length > 1) {
      ...
      if (this.options.navButtons) {
        const $controls = this.$element.find(
  ...
  _updateBullets(idx) {
    ...
    let activeStateDescriptor = $oldBullet
    ...
    if (!activeStateDescriptor.length) {
      ...
      const spanCountInOthersBullets = $othersBullets
        .toArray()
        .map((b) => $(b).children('span').length);
